...
package mediaplayer.ui.mainView;
...
public class FXMLDocumentController implements Initializable {
    ...
    private void playOrPause(MouseEvent event) {
        if(mediaPlayer!=null){
            ...
            if (mediaPlayer.getStatus()==MediaPlayer.Status.STOPPED){
                nextPlayingInd=playlistHandler.next();
                if(nextPlayingInd==-1){
                    stop(null);
                }else{
    ...
    */
    private void playFile() {
        ...
        if(nextPlayingInd==-1)return;
        Media media=new Media(mediaListView.getItems().get(nextPlayingInd).toURI().toString());
        mediaPlayer=getMediaPlayer(media);
        
    }

    private MediaPlayer getMediaPlayer(Media media) {
        if(mediaPlayer!=null)stop(null);
    ...
    private void initMediaPlayer(Media media) {
        ...
        mediaPlayer.setOnReady(()->{
            ...
            videoSeeker.setMax(mediaPlayer.getMedia().getDuration().toSeconds());
...
//            
//        } else { // this is a vedo 
//            // code to init media view is alredy in mediaplayer.onReady()
//        }
    ...
    private void loopSelector(MouseEvent event) {
        switch(getRepeatState()){
            case NO_Loop:
                ...
                prefrencesHandler.setRepeatState(RepeatState.Loop_One);
                break;
            case Loop_One:
                setRepeatStateIcon(RepeatState.Loop_All);
                prefrencesHandler.setRepeatState(RepeatState.Loop_All);
                break;
            case Loop_All:
                setRepeatStateIcon(RepeatState.NO_Loop);
                prefrencesHandler.setRepeatState(RepeatState.NO_Loop);
                break;
    ...
    private void setShuffleIcon(ShuffleState ss) {
        ...
        try{
            switch (ss) {
                case ON:
                    imageIS = getClass().getClassLoader().getResourceAsStream("mediaplayer/ui/icons/icons8_Shuffle_64px.png");
                    break;
                case OFF:
                    imageIS = getClass().getClassLoader().getResourceAsStream("mediaplayer/ui/icons/icons8_Stop_Shuffle_64px.png");
                    break;
            }
            shuffeIcon.setImage(new Image(imageIS));
    ...
    private void setRepeatStateIcon(RepeatState rs) {
        ...
        try {
            switch (rs) {
                case NO_Loop:
                    imageIS = getClass().getClassLoader().getResourceAsStream("mediaplayer/ui/icons/icons8_No_Repeat_64px.png");
                    break;
                case Loop_One:
                    imageIS = getClass().getClassLoader().getResourceAsStream("mediaplayer/ui/icons/icons8_Repeat_One_64px.png");
                    break;
                case Loop_All:
                    imageIS = getClass().getClassLoader().getResourceAsStream("mediaplayer/ui/icons/icons8_Repeat_64px_1.png");
                    break;
            }
            repeatIcon.setImage(new Image(imageIS));
        } catch (Exception ex) {
    ...
    private double[] normalModeAnchors;
    private PauseTransition uiCtrlBtnsTimer;
    
    @FXML
    private void switchFullscreen(ActionEvent event) {
        
        if(myStage.isFullScreen()){  // sitch to normal mode
            
            myStage.setFullScreen(false); // this will cla the code in lisnerto work
        }else{ // sitch to fullscreen mode
            normalModeAnchors=new double[4];
            playlistPane.toBack();
            normalModeAnchors[0]=AnchorPane.getTopAnchor(viewerPane);
            normalModeAnchors[1]=AnchorPane.getBottomAnchor(viewerPane);
            normalModeAnchors[2]=AnchorPane.getLeftAnchor(viewerPane);
            normalModeAnchors[3]=AnchorPane.getRightAnchor(viewerPane);
            AnchorPane.setTopAnchor(viewerPane, 0.0);
            AnchorPane.setLeftAnchor(viewerPane, 0.0);
            AnchorPane.setRightAnchor(viewerPane, 0.0);
            // to hide uiControlsBtns in the begining
            uiControlBtnPane.toBack();
            AnchorPane.setBottomAnchor(viewerPane, 0.0);
            // hide mouse pointer
            viewerPane.setCursor(Cursor.DISAPPEAR);
            
            uiCtrlBtnsTimer=new PauseTransition(Duration.seconds(5));
            uiCtrlBtnsTimer.setOnFinished((e) -> {
                uiControlBtnPane.toBack();
                AnchorPane.setBottomAnchor(viewerPane, 0.0);
                // hide mouse pointer
                viewerPane.setCursor(Cursor.DISAPPEAR);
            });
            viewerPane.setOnMouseMoved((ev) -> { // show ui controls btns
                // show mouse pointer
                viewerPane.setCursor(Cursor.DEFAULT);
                uiCtrlBtnsTimer.playFromStart();
                if(ev.getSceneY()>myStage.getHeight()-(63+20)){
                    uiControlBtnPane.toFront();
                    AnchorPane.setBottomAnchor(viewerPane, normalModeAnchors[1]);
  
                }
            });
            myStage.fullScreenProperty().addListener((observable, oldValue, newValue) -> {
                if(!newValue){
                    // NOTE: order materes
                    AnchorPane.setTopAnchor(viewerPane, normalModeAnchors[0]);
                    AnchorPane.setBottomAnchor(viewerPane, normalModeAnchors[1]);
                    AnchorPane.setLeftAnchor(viewerPane, normalModeAnchors[2]);
                    AnchorPane.setRightAnchor(viewerPane, normalModeAnchors[3]);
    ...
    private void showAbout(ActionEvent event) {
        ...
        String about="  This program can play Video / Audio Files \n"
                + "This project introduce me to playing audio/video in JavaFx  \n\n"
    ...
    public void userClickedOnFileInML(String filepath) {
        stop(null);
        int selFileInd=0;
        for(File f:mediaListView.getItems()){
            if(f.getAbsolutePath().equals(filepath)){
                break;
            }
            selFileInd++;
        }
        nextPlayingInd=selFileInd;
        playFile();
    }
